 ...
 * +--------------------------------------------------------------------+
	...
	HTML Sniffer is by Squiz Pty Ltd
	aXe is by Deque
...
/*
	This gets all the errors via HTMLCS.
*/


function getErrors(){
    var count = 0;
    while(HTMLCSAuditor.getIssue(count) != null) {
    	
    	//Get the error objects (May differ depending on API)
    	var retrievedData = HTMLCSAuditor.getIssue(count);

    	//Transfer relevant information
    	var ratting = retrievedData.type;
    	var description = retrievedData.msg;
    	var dom = retrievedData.element;
    	...
    	//Collect the errors
    	_errors.push(new error(ratting, description, dom, cssInfo));
...
function showAllErrorInformation(){
	...
	for(var i = 0; i < _errors.length; i++){
		list = list + " Error "+ (i+1) + ".) " + _errors[i].description + " <br>";
	}
